export default class ArrayBufferConverter {
  ...
  toString() {
    if (!this.buffer) {
      throw new Error("Буфер не загружен, воспользуйтесь командой load(тут укажите буфер, который стоит переконвертировать в строку)")
    }
  ...
  ConvertString() {
    if (!this.stringJSON) {
      throw new Error("Строка не готова, сначала вызовите toString()")
    }
    try {
